...
- Write the body to a `mktemp` file, never a fixed path — `gh pr create` and `gh pr edit` will ship a leftover file from an earlier run without complaint.
...
| Reusing a fixed path for the PR body | `mktemp` a fresh file for every run |
