...
metadata:
    github-path: .agents/skills/mino-design-by-contract
    github-ref: refs/tags/0.9.0
    github-repo: https://github.com/my-take-dev/inspired-mino-design-skills
    github-tree-sha: 63487768307e70a5d2f0d214c2be76a8e1611cd9
...
- platform差が契約またはtest実行を分岐させる場合だけ、environment conditionとWindows / Linux / macOS test matrix
...
- 業務上同じoperationは、Windows、Linux、macOSで同じprecondition、postcondition、invariant、failure guaranteeを使う。
...
- 一部platformだけのtest成功から、required platform全体の契約適合を推定している。
...
- 複数platform対応がrequiredなら、同一oracleによるplatform別test結果または未実行理由がある。
